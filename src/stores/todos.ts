...
export const useTodosStore = defineStore('todos', () => {
  ...
  function reset(): void {
    applicationPending.value = 0
    showcasePending.value = 0
    lastFetchedAt = 0
  }
  ...
  async function refresh(force = false): Promise<void> {
    ...
    try {
      ...
      applicationPending.value =
        Number(submitted.total || 0) + Number(assessing.total || 0)
      showcasePending.value = Number(showcases.total || 0)
