...
订单管理页调用 `/api/v1/manage/orders`，筛选工具栏、数据表格与分页为 Naive UI。支持服务端分页（默认每页 10 条，可切换条数并跳页）、订单状态筛选、买家用户 ID 精确筛选，点击「查询」提交、可「重置」。列表展示订单 ID、买家 ID、商品摘要（多项显示“等 N 项”）、商品件数、总金额、状态和创建时间。状态以中文标签与语义色展示（草稿/待支付/已支付/处理中/已发货/已完成/已取消/已退款）。
...
商品管理页调用 `/api/v1/manage/products`，筛选工具栏、数据表格与分页为 Naive UI。支持服务端分页、上下架状态筛选、详情抽屉、新增与编辑。列表展示商品 ID、主图、名称、分类 ID、价格和上下架状态；顶部统计当前查询结果总量与当前页上/下架数。
