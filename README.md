...
项目采用 Vue 3、TypeScript、Vite 和 Naive UI 构建，界面与业务结构参考了 [Guyungy/Kuangniao](https://github.com/Guyungy/Kuangniao)，并重新设计为更轻量、统一的现代化管理后台。Naive UI 目前处于第一阶段试点，仅迁移订单管理页的高交互组件。
...
`/orders` 是 Naive UI 第一阶段迁移试点，使用统一主题下的搜索框、状态下拉框、按钮、数据表格、分页、表单弹窗、删除确认和消息提示。关键词与状态筛选会即时更新当前本地列表；新增、编辑、删除、刷新和 CSV 导出继续复用现有列表逻辑。宽表格通过表格内部横向滚动展示，不会撑开页面主体。
...
商品、财务、报表和系统设置页面继续使用原有通用组件，并支持以下本地模拟操作；订单页提供相同操作能力，但已改用 Naive UI 组件呈现：
