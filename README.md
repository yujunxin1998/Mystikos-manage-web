...
> 当前版本已接入 Mystikos Server API 的登录认证、用户管理、陪玩师管理、陪玩申请、陪玩名片审核及商城创建订单接口；总览、订单列表、商品、财务和报表仍使用本地模拟数据，其列表增删改、筛选和导出仅在浏览器端完成。系统设置入口已暂时屏蔽。
...
| 陪玩名片审核 | 已接入展示卡分页查询与审核结果录入；详情抽屉支持页内预览封面/照片与内嵌音视频 |
| 订单管理 | 已接入服务端分页、状态/买家筛选、详情抽屉与状态流转；筛选/表格为 Naive UI |
| 商品管理 | 已接入服务端分页、状态筛选、详情、新增、编辑与图片上传；筛选/表格为 Naive UI |
| 财务、报表 | 已完成列表和本地交互；列表经通用 DataTable 使用 Naive UI |
| 操作日志 | 已接入服务端分页与操作人/路径/时间范围筛选；筛选/表格为 Naive UI |
...
- 运营总览问候语按待办数量展示，营收与订单图表标明为演示数据
- 今日营收、进行中订单、新增会员和陪玩数量统计（演示数据）
- 近 7 天、近 30 天营收趋势切换
- 今日订单状态分布与完成率展示（演示数据）
- 最新订单列表与关键字筛选，可跳转订单管理
- 创建订单表单弹窗（演示，提交后提示不落库）
- 顶栏页面跳转：输入模块名回车或点选即可进入对应页面
...
- 订单管理服务端分页、状态/买家筛选、详情抽屉与状态流转（开始处理、发货、完成、取消、退款）
- 商品管理服务端分页、状态筛选、详情抽屉、新增、编辑与图片上传（存 objectKey，展示时解析预签名地址）
- 操作日志服务端分页与操作人/路径/时间范围筛选，请求体与参数行内展开
...
字号刻度（CSS 变量）：`12 / 13 / 14 / 16 / 24 / 26`（`--app-font-xs` … `--app-font-display`）。

### 设计 Token

主题色、表面色、侧栏色、阴影与圆角统一由 `src/theme/tokens.ts` 导出，经 `createThemeCssVars` 注入 `.theme-root`：

| 类别 | 变量示例 |
| --- | --- |
| 品牌色 | `--app-primary` / `--app-primary-hover` / `--app-primary-pressed` / `--app-primary-soft` |
| 圆角 | `--app-radius-sm` 8px · `--app-radius-md` 10px · `--app-radius-lg` 12px |
| 间距 | `--app-space-1`…`--app-space-8`（4px 网格） |
| 控件高度 | `--app-control-height` 40px · `--app-control-height-sm` 32px |
| 阴影 | `--app-shadow` / `--app-shadow-subtle` / `--app-shadow-elevated` / `--app-shadow-primary` |
| 侧栏 | `--app-sidebar-*`（深色导航，浅/深主题各有一组） |

Naive UI 主题覆盖（`src/theme/naive.ts`）与上述 token 对齐。业务 CSS 应优先引用变量，避免硬编码品牌紫。
...
| 变量 | 说明 | 开发默认值 |
| --- | --- | --- |
| `VITE_API_BASE_URL` | 浏览器请求基础地址；开发环境留空以使用同源代理，生产环境配置为接口域名 | 开发环境为空 |
| `VITE_API_PROXY_TARGET` | Vite 开发代理目标地址 | `https://www.joinmystikos.com` |
...
├── src/
│   ├── api/                 # Axios 实例与接口封装
│   ├── components/          # 通用组件（表格、分页、可复制编号、统计卡片、状态标签、表单弹窗）
...
顶栏搜索用于跳转业务页面，不是全站数据检索。聚焦后会列出侧栏模块，输入「用户」「订单」「申请」等关键词可过滤，方向键选择、回车或点击即可进入对应页面。Esc 关闭下拉。

### 订单搜索

在总览“最新订单”区域输入订单编号、会员昵称、游戏名称、服务内容或陪玩师名称，可以即时筛选表格数据。点击「全部订单」或订单概览的「查看全部」会进入订单管理页。

### 创建订单

点击总览右上方的“创建订单”按钮可以打开表单弹窗。确认后会提示「演示环境不会保存订单」，不会写入服务端或本地订单列表。

### 订单管理

订单管理页调用 `/api/v1/manage/orders`，筛选工具栏、数据表格与分页为 Naive UI。支持服务端分页（默认每页 10 条，可切换条数并跳页）、订单状态筛选、买家用户 ID 精确筛选，点击「查询」提交、可「重置」。列表展示订单 ID、买家 ID、商品摘要（多项显示“等 N 项”）、商品件数、总金额、状态和创建时间。状态以中文标签与语义色展示（草稿/待支付/已支付/处理中/已发货/已完成/已取消/已退款）。详情抽屉顶部集中展示订单号、状态和总额，并将买家、下单时间、商品数量、收货信息与商品明细分区呈现；可用的状态流转操作固定在抽屉底部。

点击「查看」打开右侧详情抽屉，通过 `/api/v1/manage/orders/{orderId}` 加载最新数据，展示订单行明细（商品 ID、名称快照、单价、数量、小计）、收货地址、总金额和创建时间。抽屉内按状态流转矩阵展示可用操作：`PAID` 可开始处理/取消/退款，`FULFILLING` 可发货/退款，`SHIPPED` 可完成/退款，`COMPLETED` 可退款；取消与退款使用危险操作确认并提示会释放预占库存。操作成功后用服务端返回的最新订单更新详情并重新拉取列表。

后台不提供创建、编辑或删除订单的能力，页面不再展示这些入口。
...
「新增用户」和行内操作（角色管理、查看权限、封禁、删除）会按当前登录用户的权限编码动态显示。登录后会请求 `GET /api/v1/manage/users/{userId}/permissions`；识别 `user.create` / `user.role` / `user.permission` / `user.ban` / `user.delete`（兼容 `USER_CREATE`、`users:ban` 等写法）。权限列表为空时，仅管理员角色回退为显示全部操作，考核员等角色不会看到无权限按钮。
...
陪玩名片审核为独立页面 `/companion-showcases`，对接 [陪玩名片审核 API](https://www.joinmystikos.com/doc.html#/%E5%90%8E%E5%8F%B0%E7%AE%A1%E7%90%86%20%C2%B7%20Manage/%E5%90%8E%E5%8F%B0%E7%AE%A1%E7%90%86%20-%20%E9%99%AA%E7%8E%A9%E5%90%8D%E7%89%87%E5%AE%A1%E6%A0%B8/review)：

名片详情使用宽屏右侧抽屉：顶部集中展示标语、可约时间和游戏标签，封面在带背景衬托的媒体舞台中完整显示，照片使用缩略图轨道；下方按“简介与音视频 / 联系及审核资料”双栏组织。语音介绍使用微信式气泡播放器，支持整条点击播放或暂停、动态声波、时长和播放进度，并限制同一时间只播放一条。待审核名片可在固定底部操作区直接选择驳回或通过，窄屏时自动切换为单栏。
...
默认筛选「待审核」，支持关键词、状态、创建时间范围、分页条数切换与跳页。点击「查看」打开右侧详情抽屉：封面与照片使用 Naive UI 图片预览（页内放大，可在组内切换），音频/视频以内嵌播放器直接试听试看，不再跳转新窗口；待审记录可继续打开审核弹窗填写意见后通过或驳回。

### 其他业务列表

财务和报表页面通过通用 `DataTable` / `ModulePage` 使用 Naive UI 筛选、表格与导出，并支持以下本地模拟操作：
...
商品管理页调用 `/api/v1/manage/products`，筛选工具栏、数据表格与分页为 Naive UI。支持服务端分页、上下架状态筛选、详情抽屉、新增与编辑。列表展示商品 ID、主图、名称、分类 ID、价格和上下架状态；顶部统计当前查询结果总量与当前页上/下架数。详情抽屉使用主图与核心信息双栏概览，并将商品说明和图片画廊独立分区；窄屏自动切换为单栏。

新增与编辑使用表单弹窗：名称与价格必填，分类 ID 可选（正整数），价格须大于零；新增时需填非负整数初始库存，编辑时可调整上下架状态。商品图片通过 `POST /api/v1/files/upload` 上传，`images` 字段存储 objectKey，展示时经 `GET /api/v1/files/url` 解析为预签名地址；无法加载的图片显示统一占位。

### 商城订单创建

管理员后台不提供创建订单入口；订单由用户在前台商城通过 `POST /api/v1/orders` 从购物车下单创建，后台仅负责查看与状态流转。

## 登录说明

访问任意后台页面时，若未登录会自动跳转到 `/login`。

登录时先调用 `GET /api/v1/auth/public-key` 获取 `keyId` 和 X.509 PEM 公钥，再使用浏览器 Web Crypto API 按 RSA-OAEP + SHA-256 加密密码。随后调用 `POST /api/v1/auth/login`，只提交 `keyId` 与 Base64 格式的 `encryptedCredential`，不发送明文密码。是否启用该加密由 `src/config.ts` 的 `appConfig.loginEncryptionEnabled` 控制：设为 `false` 时跳过公钥获取与加密，直接提交明文密码 `credential`（需与后端登录加密开关保持一致）。登录成功后，Access Token、Refresh Token 和用户信息会写入浏览器 `localStorage` 或 `sessionStorage`（取决于是否勾选“记住登录状态”）。顶部头像与总览问候会再请求 `GET /api/v1/auth/me`（角色）与 `GET /api/v1/profile/me`（昵称等资料）展示当前登录用户；无昵称时回退为手机号/邮箱或用户 ID。Access Token 失效时会调用 `POST /api/v1/auth/refresh-token` 刷新；刷新失败则清除登录状态并返回登录页。顶部头像菜单调用服务端退出接口并清除本地状态。
...
- [x] 接入订单和商品管理 API
- [ ] 接入财务 API
- [x] 完善订单状态流转（开始处理、发货、完成、取消、退款）
- [x] 增加关键词搜索、状态筛选和 CSV 导出
- [x] 接入服务端分页（用户、陪玩师、申请、名片、订单、商品、操作日志）
- [ ] 批量操作
- [x] 保存主题与侧边栏设置
...
### 2026-09-03（商品与订单详情抽屉重构）

- 商品详情重排为主图与核心信息概览、商品说明和图片画廊，补充无图状态与窄屏布局
- 订单详情增加订单摘要、买家/时间/件数信息网格和独立收货区，状态操作固定在底部
- 统一两个抽屉的分区、间距、圆角、弱背景与金额层级，并补充组件结构回归测试

### 2026-09-03（复制按钮控制台警告修复）

- 移除编号复制按钮互斥的 `text` 与 `quaternary` 属性组合，避免登录后列表渲染时产生 Naive UI 警告

### 2026-09-03（接口域名切换与开发代理修复）

- 生产环境的浏览器接口基础地址切换为 `https://www.joinmystikos.com`
- 开发环境继续使用同源 `/api` 请求，并通过 Vite 代理到新域名，避免本地调试时触发浏览器跨域限制
- API 文档链接同步使用新域名
...
### 2026-08-26（UI Token 统一与一致性修复）

- 补齐 `--app-primary` 等品牌色 CSS 变量，侧栏/阴影/圆角/间距收口到 `tokens.ts`
- 字号刻度收敛为 12–26，消除 `typography.css` 与 `layout.css` 冲突
- 去掉桌面 `min-width: 1180` 硬锁，增加 1024 中档响应式；表单控件统一 40px 高度
- 业务样式中的硬编码品牌紫改为 token，卡片圆角统一为 12px

### 2026-08-26（登录页编辑风重设计与商城下单）

- 登录页改为无主图的高级科技编辑风，新增超大品牌字标、抽象轨道、系统能力栏和安全登录舱，保留全部真实认证交互
- 订单管理的创建入口接入 `POST /api/v1/orders`，按 OpenAPI 契约提交 `shippingAddress` 并展示返回订单 ID
- 明确订单列表仍为本地演示数据，避免将真实创建接口与模拟列表状态混淆

### 2026-08-26（名片详情媒体优先重设计）

- 名片详情抽屉扩展为 840px，并以完整封面媒体舞台和照片缩略图轨道作为视觉重心
- 简介、音视频与联系审核资料改为响应式双栏，修复竖版封面被压缩成细条的问题
- 待审核名片底部直接提供“驳回”和“通过审核”，减少一次操作跳转

### 2026-08-26（名片语音播放优化）

- 收紧名片详情资料区的信息层级，将语音介绍调整为独立内容块
- 使用自定义语音气泡替换浏览器原生音频控件，增加播放状态、动态声波、时长及进度反馈
- 修复详情内容区使用 `main` 标签继承应用壳层偏移而产生的大块空白，头部升级为头像、昵称、联系方式及审核状态组合栏
- 语音气泡进一步收缩为随时长变化的微信式短消息，并改用分层扩散声波反馈

### 2026-08-26（壳层与总览交互整理）

- 顶栏搜索改为页面跳转（匹配侧栏模块名），去掉无效的帮助入口和不可点下拉
- 窄屏菜单可展开侧栏；总览去掉英文日期、挥手和编造订单数，问候语改为真实待办
- 业务页去掉英文 eyebrow；统计卡和表格去掉无动作图标；演示图表标明数据来源

### 2026-08-26（用户管理按权限显示操作）

- 登录后拉取当前用户权限编码，用户列表的新增/角色/权限/封禁/删除按权限动态显示
- 权限为空时仅管理员回退显示全部操作，避免考核员等角色看到无权按钮

### 2026-08-26（表格编号一键复制）

- 所有列表中的 ID / 编号列统一使用可复制组件：悬停显示完整值，点击图标复制
- 覆盖用户、陪玩师、陪玩申请、名片审核、订单、总览最新订单，以及商品/财务/报表通用表

### 2026-08-26（名片列表列宽错乱）

- 为修订ID、用户、联系方式、标语设置足够列宽与省略，避免文字叠在一起
- 标签改为最多显示 2 个并附数量，操作列收窄，表格单行展示
- 修订ID 悬停显示完整编号，右侧复制图标点击后写入剪贴板

### 2026-08-26（名片详情清爽重排）

- 详情抽屉改为：标语身份区 → 16:9 完整封面 → 照片缩略图 → 简介/资料 → 音视频
- 封面按横向比例完整显示（不再竖裁）；照片固定 4:3 缩略图，点击仍页内预览

### 2026-08-26（名片详情页内预览）

- 陪玩名片详情改为右侧抽屉，信息与媒体分栏展示
- 封面/照片改用 Naive UI 图片预览，音频与视频改为页内播放，不再依赖新窗口打开链接

### 2026-08-26（退出登录修复）

- 退出时先清除本地登录态再请求服务端，并正确 `await` 后跳转登录页，避免守卫拦回后台并短暂显示「访客」
- 无角色时不再回退为「访客」文案；令牌过期事件同步清空 Pinia 会话

### 2026-08-26（登录页样式改版）

- 登录页改为左右分栏品牌展示 + 表单布局，窄屏回退为品牌条 + 全宽表单
- 背景改为 zinc 中性暗色，品牌紫仅用于 accent（切换条、焦点环、主按钮），减少紫雾光晕玻璃拟态
- 登录交互与接口逻辑不变：手机号/邮箱、记住登录、校验、自动填充与错误提示均保留
...
- 开发环境通过 Vite 同源代理连接 `https://www.joinmystikos.com`，生产环境直接使用该域名作为接口基础地址
...
### 2026-08-27（订单 / 商品 / 操作日志 / 文件上传接入后端）

- 订单管理接入 `/api/v1/manage/orders` 分页、详情与五种状态流转（开始处理/发货/完成/取消/退款），移除本地模拟与管理端下单入口
- 商品管理接入 `/api/v1/manage/products` 分页、详情、新增与编辑，图片经 `POST /api/v1/files/upload` 上传并存储 objectKey
- 新增操作日志页 `/operation-logs`，接入 `/api/v1/manage/operation-logs` 分页查询
- 新增文件上传 API 模块（上传 / 预签名地址 / 删除），商品图片展示时按 objectKey 解析预签名地址
